...
no root path at all and reproduced independently. **Next: pr.4** (the `/report` deploy hook, which
retires the `/qa` placeholder — it needs exactly the containers pr.2 now provides), then pr.5
(runner host), then pr.6 (lockout). Recent history follows.
